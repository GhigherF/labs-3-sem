...
namespace  Flexer
{
	...
	void Exe(LT::LexTable& lextable, IT::IdTable& idtable, In::ININ in) {
		...
		for (int i = 0; i < size; i++) {
			if (words[i].second != currLine) {
				currLine++;
				cout << "\n";
				cout << setw(3) << setfill('0') << currLine << " ";
		...
		cout<<"   id      iddatatype    idtype     idxfirstLE    value" << '\n';
		cout << "---------------------------------------------------------" << '\n';
		...
		for (int i = 0; i < idtable.size; i++) {
			...
			if (ent.idtype == IT::V)
			{
				cout << setw(7) << setfill(' ') << "V";
			}
			else if (ent.idtype == IT::F) {
				cout << setw(7) << setfill(' ') << "F";
			}
			else
			{
				cout << setw(7) << setfill(' ') << "P";
			}
